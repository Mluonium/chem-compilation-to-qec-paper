...
def add_gridsynth_rz(qc, target, theta, bits):
    ...
    circuit = gridsynth_circuit(
        theta=theta, epsilon=epsilon, wires=[0], up_to_phase=True
    )  # actual gridsynth circuit
...
def decompose_qpe_with_gridsynth(qc, bits):
    ...
    for gate in qc.gates:
        if gate.name == "RZ":
            ...
            add_cphase_decomposition(
                new_qc, gate.controls[0], gate.targets[0], gate.arg_value, bits
            )
        else:
            new_qc.add_gate(
                gate.name,
...
def swap_operator(
    a, b, N
):  # takes index of qubits to be swapped and total number of qubits
    X = q.sigmax()
    Y = q.sigmay()
    Z = q.sigmaz()
    I = q.qeye(2)
...
def embed_two_qubit_gate(U, q1, q2, N):
    ...
    for input_index in range(dim):
        input_bits = [
            (input_index >> (N - 1 - i)) & 1 for i in range(N)
        ]  # binary qubit values
        local_input = (
            2 * input_bits[q1] + input_bits[q2]
        )  # takes the 2 selected qubits and translates to 0-3 index
        ...
        for local_output in range(4):
            ...
            for bit in output_bits:
                output_index = (
                    output_index << 1
                ) | bit  # output bit string to integer index
            full_U[output_index, input_index] = U[
                local_output, local_input
            ]  # put matrix element into the bigger matrix
